...
class starter{
        ...
        if(diff==1){
            ...
            for(int r = 0; r<bombProxGrid.length; r++){
                for(int c = 0; c<bombProxGrid[0].length; c++){
                    ...
                    if((random==0)&&(bombProxGrid[r][c]!=9)){
                        bombProxGrid[r][c]=9;
                        i++;
    ...
    */
    public static void addAdj(int r, int c, int [][] bombProxGrid){
        if(r+1!=bombProxGrid.length){
            bombProxGrid[r+1][c]++;
         }
        if(r-1!=-1){
            bombProxGrid[r-1][c]++;
        }
        if(c+1!=bombProxGrid[0].length){
            bombProxGrid[r][c+1]++;
        }
        if(c-1!=-1){
            bombProxGrid[r][c-1]++;
        ...
        if(r+1!=bombProxGrid.length&&c-1!=-1){
            bombProxGrid[r+1][c-1]++;
        
        }
        if(r-1!=-1&&c+1!=bombProxGrid[0].length){
            bombProxGrid[r-1][c+1]++;
        }
        ...
        if(bombProxGrid[r][c]!=9){
            bombProxGrid[r][c]=0;
            if(r+1!=bombProxGrid.length){
             if(bombProxGrid[r+1][c]==9){ //up
                bombProxGrid[r][c]++;
             }
            }
            if(r-1!=-1){
             if(bombProxGrid[r-1][c]==9){ //down
                bombProxGrid[r][c]++;
             }
            }
            if(c+1!=bombProxGrid[0].length){
            ...
            if(r+1!=bombProxGrid.length&&c+1!=bombProxGrid[0].length){
             ...
             if(r+1!=bombProxGrid.length&&c-1!=-1){
             if(bombProxGrid[r+1][c-1]==9){ //up one
                bombProxGrid[r][c]++;
             }
             }
             if(r-1!=-1&&c+1!=bombProxGrid[0].length){
